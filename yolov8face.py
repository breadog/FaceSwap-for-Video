...
class YOLOface_8n:
    def __init__(self, modelpath, conf_thres=0.5, iou_thresh=0.4, providers=None):
    ...
    def postprocess(self, outputs):
        ...
        if keep_indices.any():
            ...
            bboxes_wh *= np.array(
                [[self.ratio_width, self.ratio_height, self.ratio_width, self.ratio_height]])  ###合理使用广播法则
            face_landmark_5_raw *= np.tile(np.array([self.ratio_width, self.ratio_height, 1]), 5).reshape(
                (1, 15))  ###合理使用广播法则,每个点的信息是(x,y,conf), 第3个元素点的置信度，可以不要，那也就需要要乘以1
            ...
            if len(indices) > 0:
                ...
                bounding_box_list = list(
                    map(lambda x: np.array([x[0], x[1], x[0] + x[2], x[1] + x[3]], dtype=np.float64),
